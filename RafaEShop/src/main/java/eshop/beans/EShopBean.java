...
public class EShopBean {
    ...
    @PersistenceContext(unitName = "RAFA_PU")
    ...
    public Map<Integer, List<Products>> extractProducts(String id, int page) {
        ...
        org.apache.lucene.search.Query query = queryBuilder.keyword().onField("category.id").matching(id).createQuery();
    ...
    public List<Products> extractPromotionalProducts() {
        ...
        org.apache.lucene.search.Query query = NumericRangeQuery.newDoubleRange("old_price", 0.0d, 1000d, false, true);
        FullTextQuery fullTextQuery = fullTextEntityManager.createFullTextQuery(query, Products.class);
        Sort sort = new Sort(new SortField("price", SortField.DOUBLE));
        fullTextQuery.setSort(sort);
